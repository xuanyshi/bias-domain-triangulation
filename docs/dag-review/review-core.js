...
function previousData(d){const p=d.previousBaseline;return p?{...d,version:p.version,graphHash:p.graphHash,nodes:d.nodes.filter(n=>p.nodeIds.includes(n.id)),previousBaseline:null}:null;}
function previousMatch(v,d){return !!d.previousBaseline&&v?.version===d.previousBaseline.version&&v?.graphHash===d.previousBaseline.graphHash;}
function upgradeBaseline(v,d,allowSnapshot){
 const original=normalize(v,previousData(d),allowSnapshot);
 function expand(record){const out={...record,version:d.version,graphHash:d.graphHash,nodes:{...record.nodes},nodeReviews:{...record.nodeReviews}};for(const n of d.nodes)if(!Object.hasOwn(out.nodes,n.id)){out.nodes[n.id]='';out.nodeReviews[n.id]='';}if(out.context)out.context=reviewPackage(out.context,d);if(out.firstRound)out.firstRound=expand(out.firstRound);return out;}
